...
const fetch: ServerFetcher<StackOverflowConfig, StackOverflowMeta> = async (config, cursor) => {
  const page = cursor ? Number(cursor) || 0 : 0;
  const r = await fetchStackOverflowPage(config.mode, config.tag, PAGE_SIZE, page);
  return {
    items: r.items,
...
export const server = defineColumnServer<StackOverflowConfig, StackOverflowMeta>({
  meta,
  fetch,
});
