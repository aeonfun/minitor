...
const fetch: ServerFetcher<DexscreenerConfig, DexscreenerMeta> = async (config, cursor) => {
  const items = await fetchDexscreenerItems(config.mode, config.query, config.watchlist);
