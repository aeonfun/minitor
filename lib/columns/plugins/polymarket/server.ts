...
const fetch: ServerFetcher<PolymarketConfig, PolymarketMeta> = async (config, cursor) => {
  ...
  const effectiveMode = config.mode === "tag" && !config.tag.trim() ? "trending" : config.mode;
  const r = await fetchPolymarketPage(effectiveMode, config.tag, PAGE_SIZE, page);
  return {
    items: r.items,
